function [ p,returna,returnb,returnc ] = Kmeans( FindTime,datetype)
...
cy=max(mt);
ax=0;bx=ceil(length(mt)/2);cx=length(mt);
for i=1:length(mt)
    if mt(i,1)==ay &&ax==0
        ax=i;
    elseif mt(i,1)==by && bx==0
        bx=i;
...
for k=1:1:3000
    ...
    for i=1:1:length(x)%E²½
        ...
        blen=((abs(x(i,1)-bx)).^0.25+(abs(y(i,1)-by)).^0.25).^4;
        clen=((abs(x(i,1)-cx)).^0.25+(abs(y(i,1)-cy)).^0.25).^4;
        if alen<blen && alen<clen
            a(aj,1)=x(i,1);
            a(aj,2)=y(i,1);
            aj=aj+1;
        elseif blen<alen && blen<clen
            b(bj,1)=x(i,1);
            b(bj,2)=y(i,1);
            bj=bj+1;
        elseif clen<alen && clen<blen
            c(cj,1)=x(i,1);
            c(cj,2)=y(i,1);
            cj=cj+1;
        end
        now=now+alen+blen+clen;
    end
    ax=mean(a(:,1));
    ay=mean(a(:,2));
    bx=mean(b(:,1));
...
title('Open source softwave(AMQ)');
returna=a(:,2);
returnb=b(:,2);
returnc=c(:,2);
end
